...
"""
Single-GPU PEFT-LoRA fine-tuning (H100 / bf16) with:
- CSV logging (train/eval loss)
- loss_curve.png saved to output_dir
- best model selection (eval_loss)
- optional merge-and-save for eval-ready full weights
"""
...
try:
    ...
    _sig = str(inspect.signature(Accelerator.unwrap_model))
    if "keep_torch_compile" not in _sig:
        ...
        print("[WARN] Patched Accelerator.unwrap_model to ignore keep_torch_compile")
except Exception as _e:
    print("[WARN] Accelerate guard not applied:", _e)
...
def get_bnb_config(load_in_4bit: bool, load_in_8bit: bool) -> Optional[BitsAndBytesConfig]:
    ...
    if load_in_4bit:
        return BitsAndBytesConfig(
            ...
            bnb_4bit_compute_dtype=torch.bfloat16,  # H100-friendly
        )
    if load_in_8bit:
